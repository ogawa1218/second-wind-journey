...
その場合のみ Step 7 でユーザーに確認する。**VO2max のサブエガ目標値は 58（必達）/ 56（最低）/ 60（ストレッチ）**。
...
例:
```
| 2026-07-04 | 18.0km | 5'31"/km | 152bpm | ロング走 | 68.5kg | 睡眠7.0h/82pt | VO2max53 | ブランク確認テスト・違和感なし |
```
...
| VO2max | {metrics.vo2max}（必達58まで{58-vo2max}） |
| 種別（判定） | {種別} |
| Day番号 | Day{DAY}/164 |
````
