...
`metrics` の各値は別 API から自動取得する。取得できなかった項目は `null` で返るので、
その場合のみ Step 7 でユーザーに確認する。**VO2max は現在地59 → ゴール目標65（中間チェック62）。59で既にサブエガ射程圏、65は盤石化の攻めの目標**。
...
### Step 7 — 追加情報を確認する（`metrics` が null の項目のみ）

体重・睡眠・VO2max は `metrics` から自動取得済み。**取得できた項目は聞かない。**
`null` の項目と、API では取れないメモのみ確認する。

```
体重（metrics.weightKg が null の場合）: ___kg（前日比 ±___kg、スタート100kgからの累計 -___kg）
睡眠（metrics.sleepScore が null の場合）: ___h・睡眠スコア___pt
VO2max（metrics.vo2max が null の場合）: ___ml/kg/min（前回値があれば併記）
メモ（任意）: ___（省略可）
```
...
**VO2max行**:
- VO2maxあり: `VO2max{N}（ゴール65まであと{65-N}）`／N≧65なら`（ゴール65到達・貯金{N-65}）`
- VO2maxなし: 行ごと省略

**考察ブロック（必須・データ→意味づけを各指標1行）**:
`/garmin-post` スキルの「考察ブロック」と同一ルール。読み取れた体重・睡眠・VO2maxそれぞれを
「行動序列ドミノ（睡眠→食事→運動）」と「VO2maxゴール65（現在地59）」に接続して観測ベースで書く。
- 体重: 64kg計画に対する位置づけ（残り{現体重-64}kgを{レースまで週数}週で／週−必要ペース）
- 睡眠: スコアと当日強度の整合（高→ポイント練GOの根拠／低→強度を落として正解）
- VO2max: ゴール65までの残り＋`理論値@64kg ≒ 現VO2max×現体重/64`＋レース日逆算で「順調/要加速/黄信号」
...
体重は残り4.5kgを約20週で、週−0.23kgペースなら計画通り。
スコア82ptは回復十分→18kmロングを入れられた根拠。
VO2max59→64kg到達時の理論値は約63、減量でゴール65が射程に。残り日数的に「順調」。
...
```
| {YYYY-MM-DD} | {distanceKm}km | {avgPaceStr}/km | {avgHR}bpm | {種別} | {体重}kg | 睡眠{X}h/{N}pt | VO2max{N} | {メモ or activityName} |
```

例:
```
| 2026-07-04 | 18.0km | 5'31"/km | 152bpm | ロング走 | 68.5kg | 睡眠7.0h/82pt | VO2max59 | ブランク確認テスト・違和感なし |
```

VO2max が取れない回は `—` を入れる（列はずらさない）。
...
| 体重 | {metrics.weightKg}kg |
| 睡眠 | {metrics.sleepH}h / {metrics.sleepScore}pt |
| VO2max | {metrics.vo2max}（ゴール65まで{65-vo2max}） |
| 種別（判定） | {種別} |
| Day番号 | Day{DAY}/164 |
````
